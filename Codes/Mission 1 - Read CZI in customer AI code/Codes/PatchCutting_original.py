...
import pandas as pd
import warnings
from czi_utils import CZIConverter
...
mount_path = f'./{Project}/{Project}'
ALL_WSI_PATH = f'./PLN/{data_set}'
RAW_DATA_PATH = f'{mount_path}/raw_data'
PATCHES_ABS_PATH = f'./ALL_DATA/{Project}/{data_set}'
...
extConverter = CZIConverter("../bftools/bfconvert")
...
def joint_patch(path, patch_size=32):
    ...
    for i in patches:
        if '.ipy' in i:
            continue
        y, x = list(map(int, i.split('.')[0].split('_')))
        arr[x * patch_size:x * patch_size + patch_size, y * patch_size:y * patch_size + patch_size] = np.array(
            Image.open(os.path.join(path, i)).resize((patch_size, patch_size), Image.ANTIALIAS))
...
for ptid in all_ptid:
    ...
    for slide in all_wsi:
        if slide.lower().endswith("czi"):
            extConverter(os.path.join(ALL_WSI_PATH, slide),
                         os.path.join(ALL_WSI_PATH, slide.lower().replace("czi", "tiff")))
            os.remove(os.path.join(ALL_WSI_PATH, slide))
            slide = slide.lower().replace("czi", "tiff")
        if ptid in slide:
            ...
            shutil.copyfile(os.path.join(ALL_WSI_PATH, slide),
...
for ptid in os.listdir(RAW_DATA_PATH):
    ...
    for slide in os.listdir(f'{RAW_DATA_PATH}/{ptid}/'):
        ends = slide.split('.')[-1]
        if ends not in ['svs', 'tif', 'SVS', 'TIF', 'TIFF', 'tiff']:
            continue
...
for ptid in ptids:
    ...
    for cases in os.listdir(os.path.join(RAW_DATA_PATH, ptid)):
        ends = cases.split('.')[-1]
        if ends in ['svs', 'tif', 'SVS', 'TIF', 'TIFF', 'tiff']:
            try:
                pg = PatchGenerator(
...
all_data = os.listdir(f'./Pediatrics/Pediatrics/patches/')
